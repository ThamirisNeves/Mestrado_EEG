...
class BCI():
    def __init__(self):
        ...
        self.train_set,self.valid_set, self.ytrain, self.yvalid = self.carrega_moaab()

    def carrega_moaab(self):
        """Load dataset"""
        train_set = np.load('/home/tneves/Documents/Projeto_mestrado/Projeto_mestrado/Codigos/suj_1_train.npy')
        valid_set = np.load('/home/tneves/Documents/Projeto_mestrado/Projeto_mestrado/Codigos/suj_1_valid.npy')
        ytrain = np.load('/home/tneves/Documents/Projeto_mestrado/Projeto_mestrado/Codigos/suj_1_y_train.npy')
        yvalid = np.load('/home/tneves/Documents/Projeto_mestrado/Projeto_mestrado/Codigos/suj_1_y_valid.npy')
        print(train_set.shape)
        
        return (train_set,valid_set, ytrain, yvalid)
...
#
    ...
    def train_val_test(self,train_set,valid_set, ytrain, yvalid):
        X_train = train_set
        Y_train = ytrain
        X_test = valid_set
        Y_test = yvalid
...
for suj in sujeitos:
    ...
    bci = BCI()
    for seg in segundos:
        df_train, df_test, ytrain, yvalid = bci.carrega_moaab()
        for param in params[seg]:
            for kernLength, F1, D , F2 in modelo:
                for batch in batch_sizes:
                    X_train,Y_train,X_validate,Y_validate,X_test,Y_test,chans,samples = bci.train_val_test(df_train,df_test,ytrain, yvalid)
